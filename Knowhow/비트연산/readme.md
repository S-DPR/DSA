...
3. AND 연산을 수행합니다.
   - 맨 앞자리는 3이 0을 갖고있으니 0이 내려옵니다.
   - 두번째 자리는 5가 0을 갖고있으니 0이 내려옵니다.
   - 세번째 자리는 3과 5 모두 1을 갖고있으니 1이 내려옵니다.
...
3. **부호비트**에도 AND연산을 수행합니다.
   - ```A < 0 && B < 0```인경우 결과값은 음수입니다.
   - ```A > 0 || B > 0```인경우 결과값은 양수입니다. 
...
2. 3이 자릿수가 적으니 앞에 0을 넣어 자릿수를 맞춰줍니다. 이제 ```011(2)```, ```101(2)```입니다.
3. OR 연산을 수행합니다.
   - 맨 앞자리는 5가 1을 갖고있으니 1이 내려옵니다.
   - 두번째 자리는 3이 1을 갖고있으니 1이 내려옵니다.
   - 세번째 자리는 3과 5 모두 1을 갖고있으니 1이 내려옵니다.
1. 결론적으로 ```111(2)```이므로, ```3|5 = 7```이라는 결과물이 나오게 됩니다.
...
3. XOR 연산을 수행합니다.
   - 맨 앞자리는 5가 1을 갖고있으니 1이 내려옵니다.
   - 두번째 자리는 3이 1을 갖고있으니 1이 내려옵니다.
   - 세번째 자리는 3과 5 모두 1을 갖고있으니 0이 내려옵니다.
...
2. **부호비트**에도 AND연산을 수행합니다.
   ...
   - ```AB > 0```인경우 결과값은 양수입니다.
3. ```A^A == 0```이 항상 성립합니다.
4. 집합론적으로, ```A^B```는 **A와 B의 합집합에서 교집합을 뺀 집합**을 뜻합니다.
...
2. NOT 연산을 수행합니다.
   - 첫번째 자리는 1이니 0이 내려옵니다.
   - 두번째 자리는 0이니 1이 내려옵니다.
...
---------------
<h3>Binary GCD</h3>
GCD를 비트연산으로 구현할 수 있습니다.

비트연산이 모듈러연산보다 빠름에서 착안한 알고리즘입니다.

정수론에 대한 내용이 포함되어있습니다.

큰 수에 대해서, 일반 GCD보다 더 빠르다고 합니다.

```
def binaryGCD(a, b):
    # a, b중 하나가 0이면 0이 아닌 값을 내보낸다.
    if not a*b: return max(a, b)

    # gcd(x, y)에서 x, y가 모두 짝수라면 2*gcd(x/2, y/2)와 같다.
    # 2는 공약수지만 최대공약수가 아니기에, 아래과정을 거쳐
    # 같음을 증명할 수 있다.
    # 짝수인 x, y를 각각 소인수분해해보면
    # x = 2 * P (2 <= P)
    # y = 2 * K (2 <= K)
    # 이므로 최대공약수는 P와 K의 최대공약수에서 2를 곱한 값이 된다.
    # 결론적으로 2를 이항하면
    # x/2 = P, y/2 = K
    # 이후 P와 K를 똑같이 x, y에 대입하면,
    # x와 y중 하나는 9이거나 홀수인 값을 얻게된다.
    # 이 과정에서 shift라는 변수에 1번 나눈 횟수당 1을 올려준다.
    # shift가 2*gcd(x/2, y/2)에서 '2*' 부분이다.
    shift = 0
    while ~(a|b)&1:
        shift += 1
        a >>= 1
    ...
    # gcd(x, y)에서 x가 짝수라면 gcd(x/2, y)가 성립한다.
    # x = 2 * P (2 <= P)
    # y = K
    # 로 소인수분해 되고 2는 x와 y의 공통약수가 아니다.
    # 그러므로 2를 이항하면
    # x/2 = P, y = K
    # P와 K의 약수중에서 최대공약수가 나오게 된다.
    while ~a&1: a >>= 1
    while ~b&1: b >>= 1

    # gcd(x, y)에서 x와 y 모두 홀수라면 gcd(|x-y|/2, y)
    """
    증명
    gcd(x, y) = d라고 하자. x, y(x > y)는 모두 홀수이므로 d는 반드시 홀수이다.
    그러면 x = dP, y = dK로 나타낼 수 있다. (단, P, K는 자연수)
    이제 x-y를 구하자. x-y는 dP-dK로 나타낼 수 있으며, d(P-K)로 묶을 수 있다.
    이 때, x, y, d가 모두 홀수이므로 P, K도 자연히 홀수이게 된다.
    P-K는 홀수-홀수이므로 짝수가 되는데, y가 홀수이므로 2는 자연스럽게 d가 아니게 된다.
    그러므로 다음과 같이 쓴다. x-y = d(P-K)/2
    결론적으로 gcd(|x-y|/2, y) = gcd(d(P-K)/2, dK) = d = gcd(x, y). 
    """
    # a와 b가 다르다면 이하 과정을 반복한다.
    while a != b:
        if a > b:
            a -= b
            while ~a&1: a >>= 1
